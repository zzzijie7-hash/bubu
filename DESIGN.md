...
1. **暗色星图** — 深色背景是星空画布，地点标记是星光。宇宙元素点到为止，不给信息层添噪
2. **一种颜色就够了** — 绿色是唯一强调色。层级靠亮度差建立，不靠颜色差
...
| Token | Hex | 用途 |
|-------|-----|------|
| `primary` | `#4ADE80` | 主强调色 — 去过标记、主要按钮、已点亮状态 |
| `primary-active` | `#22C55E` | 按压态 — 按钮按下、选中态更深绿 |
| `primary-soft` | `#166534` | 绿的低调用法 — 标签背景、非激活标记 |

### 表面层级（暗色阶梯）

| Token | Hex | 用途 |
|-------|-----|------|
| `space` | `#0A0A0F` | 最底层 — 全屏画布，类太空深黑，带极微弱蓝紫底调 |
| `surface-1` | `#12121A` | 一级浮层 — 卡片、Sheet、列表行 |
| `surface-2` | `#1A1A24` | 二级浮层 — 弹窗、Modal、突出卡片 |
| `surface-3` | `#24243A` | 三级浮层 — hover 态（极少用） |

### 文字色

| Token | Hex | 用途 |
|-------|-----|------|
| `ink` | `#F5F5F7` | 标题、正文（非纯白，略暖） |
| `ink-secondary` | `#A1A1AA` | 副文字、说明文字 |
| `ink-tertiary` | `#6B6B76` | 占位文字、禁用态、时间戳 |
| `on-primary` | `#0A0A0F` | 绿色按钮上的深色文字 |
...
| `star-dim` | `#A5B4FC` | 暗星点，带淡蓝紫 op 0.2–0.4 |
| `nebula-purple` | `#7C3AED` | 星云渐变用，仅在装饰层，op 0.03–0.08 |
| `nebula-teal` | `#2DD4BF` | 星云渐变用，op 0.03–0.08 |

### 色板使用纪律

- `primary` #4ADE80 **只用于**：去过标记、主按钮、点亮状态、选中态
...
| 角色 | 字体 | 说明 |
|------|------|------|
| **标题** | SF Pro Display | 系统原生，Regular/Semibold/Bold |
| **正文** | SF Pro Text | 小字号更清晰 |
| **数字/坐标** | SF Pro (monospaced digits) | 数字对齐 |
| **特殊装饰** | SF Pro Rounded | 仅在 Welcome 页大标题或 tag 标签用 |
...
- 标题用 SF Pro Display Regular/Semibold，永不用 Heavy/Black 字重
...
| 层级 | 处理 | 用途 |
|------|------|------|
| 0 基底 | `space` #0A0A0F | 地图画布 |
| 1 浮层 | `surface-1` #12121A | Sheet、卡片 |
| 2 抬升 | `surface-2` #1A1A24 + 0.5pt `primary` 发光边 | Modal、重要弹窗 |
| 3 最高 | `surface-3` #24243A | 极少用 |
...
#### `button-primary`
- 背景 `primary` #4ADE80，文字 `on-primary` #0A0A0F
- 字体 `button` (15pt Medium)，padding 12pt×24pt，圆角 `md` (12pt)
- 高度 48pt（≥44 HIG 标准）
- 按压态：背景变 `primary-active` #22C55E

#### `button-secondary`
- 背景透明，文字 `ink` #F5F5F7，1pt `surface-2` 描边
- 其他同 button-primary
...
#### `chip-selected`
- 背景 `primary-soft` #166534，文字 `primary` #4ADE80
- 其他同 chip-default
...
#### 地图上的绿色点 = "去过"
- 系统 map pin marker（圆形），fill `primary` #4ADE80
- 绿色标记 = 已点亮
...
- 绿色 #4ADE80 在真机暗色屏幕上的实际观感（模拟器不准，需真机验证）
